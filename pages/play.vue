  ...
  <div class="index-wrap">
    ...
    <div class="main" v-show="counting == 0">
      <div class="bear-hand" 
        ...
        :style="{backgroundPosition: bearHandPosition, height: bearHeight + 'px', top: bearTop + 'px'}"
      >
      ...
      <transition>
        <div class="btns animate__animated animate__bounceInUp" v-if="showBtns">
          <div class="yzlw" @click="goMake"></div>
          <div class="slwq"></div>
        </div>
      </transition>
      <!-- <video id="alpha-video" autoplay muted   webkit-playsinline playsinline  preload> -->
      <!-- <video id="alpha-video" :src="giftVideoURL" style="display: none;" muted loop x5-video-player-type="h5"  webkit-playsinline playsinline  preload >
        <source :src="giftVideoURL" type="video/mp4" >
      </video> -->
      <video style="position:absolute;left:0;bottom:0;height: 1px; width: 1px;background:#000;" muted  loop autoplay id="alpha-video" :src="giftVideoURL" x5-video-player-type="h5" webkit-playsinline playsinline  preload  >
        <source :src="giftVideoURL" type="video/mp4" >
      </video>
      <div id="output">
...
export default {
  data() {
    return {
      ...
      slideDirection: '',
      giftVideoURL: '',
      // giftVideoURL: 'https://img.qianhuituan.cn/uploads/videos/202008/20/KozmwgWVh7yYHlJUR5L2CeBa4bGgu3Whsps7YMot.mp4',
      canvas: {
        showCanvas: null,
        showCtx: null,
        bufferCanvas: null,
  ...
  mounted() {
    ...
    this.bearTop =  this.bearHeight > this.screenHeight / 2 ?  this.screenHeight / 2 - this.bearHeight + 80 : 0
  ...
  methods: {
    initGift(){
      ...
      if(gift && this.videosList.includes(gift)){
        this.giftVideoURL = require(`./video/${gift}.mp4`)
        return
      }
      const index = Math.floor(Math.random() * this.videosList.length)
      this.giftVideoURL = require(`./video/${this.videosList[index]}.mp4`)
    },
    initCanvas(){
      ...
      this.canvas.video.addEventListener('play', (event) => {
        this.processFrame()
      });
      // ios微信无法播放视屏问题修复
      document.addEventListener("WeixinJSBridgeReady", () => {  
        this.$nextTick(() => {
          this.canvas.video.play()
        })
      });
    },
    startCount(){
    ...
    onTouchEnd(e){
      const endY = e.changedTouches[0].clientY
      const isRightDirection = endY - this.moveStartY > 0 && this.slideDirection == 'down' || endY - this.moveStartY < 0 && this.slideDirection == 'up'
      this.moveDistance = Math.abs(endY - this.moveStartY)
      if(this.moveDistance >= openDistance && isRightDirection){
        if(this.openStep == 2){
          this.showBtns = true
          this.showGift = true
          this.continueFinish()
    ...
    playGift(){
      // this.canvas.video.addEventListener('oncanplay', () => {
        this.canvas.video.play();
      // })
    },
    goMake(){
